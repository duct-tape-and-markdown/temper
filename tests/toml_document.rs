...
/// A dial document: scalars, an array, and a sub-table — the shapes a TOML top level puts
/// in a member's fields, including the nested-table-as-object the read must keep whole for
/// the shared fold to type.
const DIAL_TOML: &str = r#"name = "workstation"
mode = "advisory"
gates = ["fmt", "clippy"]
...
/// The fixture kind: a `toml-document` file kind governing the dial locus, its identity read
/// from the document's top-level `name`.
fn dial_kind() -> CustomKind {
...
fn dial_kind_row() -> KindFactRow {
    KindFactRow {
        format: Some("toml-document".to_string()),
        unit_shape: Some("named-field(name)".to_string()),
        ..common::kind_facts("dial", ".claude/local", "*.toml")
    }
}

/// Write `body` at the dial locus under `root`.
fn write_dial(root: &Path, body: &str) -> PathBuf {
    let path = root.join(".claude/local/dial.toml");
...
fn a_toml_document_reads_its_top_level_keys_as_the_members_fields() {
    ...
    let path = write_dial(&dir, DIAL_TOML);

    let member = toml_document::read(&dial_kind(), &path).unwrap();
...
fn identity_is_read_from_the_declared_field_never_the_filename_stem() {
    ...
    // The stem is `dial` for every dial ever written; the declared key is what distinguishes
...
fn a_document_that_is_not_valid_toml_refuses_loud() {
    ...
    let path = write_dial(&dir, "name = \"workstation\"\nmode =\n");

    let err = toml_document::read(&dial_kind(), &path).unwrap_err();
...
fn the_declared_format_decides_which_adapter_reads_the_artifact() {
    ...
    write_dial(&dir, DIAL_TOML);
    common::write_lock(
        &dir,
        Declarations {
            kinds: vec![dial_kind_row()],
    ...
    assert!(run.ok, "a conforming dial gates clean: {}", run.output);
    assert!(
...
fn the_dispatch_still_judges_a_toml_document_that_omits_a_required_field() {
    ...
    write_dial(&dir, "name = \"workstation\"\n");
    common::write_lock(
        &dir,
        Declarations {
            kinds: vec![dial_kind_row()],
...
fn a_composed_toml_document_member_refuses_at_emit_rather_than_being_written() {
    ...
    assert!(message.contains("dial:workstation"), "{message}");
    ...
    let payload = Payload {
        ...
        declarations: Declarations {
            ...
            includes: vec![temper::drift::IncludeRow {
                member: "dial:workstation".to_string(),
