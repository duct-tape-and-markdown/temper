//! The nested-member display rule's byte contract (`specs/architecture/20-surface.md`,
...
fn a_double_render_is_byte_identical() {
    // Determinism is the contract (law 5): the same member renders the same bytes, the
