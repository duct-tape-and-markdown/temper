...
[requirement.member]
means = "the `member` concept — one artifact instance in one of three carriages (module-carried in the SDK, document-carried in a migration-era corpus, in-place in the landscape), one feature shape however carried — must be given an architecture home"
kind = "architecture"
required = true
...
means = "the `assembly` concept — the layered binding declaration, authored in the SDK, its locusless facts emitted as committed temper-owned artifacts — must be given an architecture home"
kind = "architecture"
required = true

[requirement.authoring-face]
means = "the SDK — the typed module library (members, kinds, packages, the assembly as typed values) that emit compiles into the harness artifacts — must be given an architecture home"
kind = "architecture"
required = true
...
source_hash = "4979c7a957259933bbcaf0a2c1eb636b1f7912d1c815789c465f6aada836aaa1"
+++
