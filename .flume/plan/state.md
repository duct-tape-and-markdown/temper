...
- Posture swept through: mid-rotation, at src/document.rs — clean;
  `src/drift.rs` next in the c9d11d5 re-arm rotation's frontier.
- This tick: POSTURE SWEEP src/document.rs (no internal `use crate::`
  imports — external crates only, so no immediate-import neighbor) —
...
Plan continues: yes — the posture rotation is open (drift.rs onward
