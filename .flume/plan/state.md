...
- Posture swept through: sdk/src/ tree fully covered; src/address.rs, src/admissibility.rs,
  ...
  src/compose.rs covered. src/contract.rs next in tree order — mid-rotation.
- This tick: POSTURE SWEEP src/compose.rs — one stale cite found (`manifest_units` doc
  comment, line 556, still names `Manifest::read_kind`; superseded by
  GATE-MANIFEST-SHARED-READ-HOIST's `ManifestCache` read path) — routed to open-questions.md's
  ride-only orphan list per the stale-cite exception, not a pending entry; see commit body.
...
open posture rotation drives itself next tick, resuming at src/contract.rs.
