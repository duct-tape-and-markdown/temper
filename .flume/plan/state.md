...
- **Phase:** contract-engine cutover. The generic `Contract` model + decidable
  engine (`src/contract.rs`, `src/engine.rs`), the skill extractor
  (`src/extract.rs::skill_features`), and the bundled Anthropic skill template
  (`contracts/skill.anthropic.toml`) are all shipped; the rejected heuristic
  registry is still wired into `check` and awaits the two-entry cutover chain.
- **Last shipped:** SKILL-CONTRACT-TEMPLATE (14dee1c + 5793d44) — the bundled
  template loads through `Contract::parse` with its clause vector asserted
  (`tests/contract_template.rs`).
...
accurate (cited APIs `skill_features`, `validate`, `render`/`any_error`,
`Contract::parse` and `contracts/skill.anthropic.toml` all present; `check` still
on `rules::all_rules()` at main.rs:57; every retire target present), inbox empty,
open-questions current. The queue is unchanged and a pickable `open` entry leads
the chain — build drains it.
