...
- Posture swept through: mid-rotation, at src/path.rs — clean;
  src/placement.rs next in the c9d11d5 rotation's frontier.
- This tick: POSTURE SWEEP src/path.rs — clean, no findings (evidence in
  the commit body).
...
non-empty: src/placement.rs onward) with no pickable entry in the queue
