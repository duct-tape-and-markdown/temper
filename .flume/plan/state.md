...
- **Phase:** contract-engine cutover. The generic `Contract` + decidable engine
  ...
  (`extract::skill_features`), and the bundled Anthropic skill template
  (`contracts/skill.anthropic.toml`) are all shipped on disk; the rejected
  heuristic registry is still wired into `check` (main.rs:57) awaiting the
  two-entry cutover chain.
- **Last shipped:** SKILL-CONTRACT-TEMPLATE (14dee1c) — bundled template loads
  through `Contract::parse` with its clause vector asserted.
- **In flight:** nothing committed. Working tree carries only a cosmetic import
  ...
  main.rs/cli.rs/acceptance.rs still cite the absent `spec/RELEASE-v0.1.md` — all
  CHECK-CUTOVER targets. Plan never edits `src/`.
...
main.rs:57; every retire target present). Corrected one line-number drift in
CHECK-CUTOVER (`use temper::rules;` is main.rs:17, not :16); inbox empty,
